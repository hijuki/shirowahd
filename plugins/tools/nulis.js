...
const fontUrl = getAssetBuffer("hillz-font");
...
async function handler(m, { sock }) {
  ...
  const inputUrl = getAssetBuffer("hillz-kertas");
  if (!inputUrl) {
    return m.reply(
  ...
  try {
    ...
    if (!_fontRegistered) {
      try {
        const fontBuf = getAssetBuffer("hillz-font");
        if (fontBuf) {
    ...
    const bgBuf = getAssetBuffer("hillz-kertas");
