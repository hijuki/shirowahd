...
async function startAutoJpmSession(m, sock, db) {
  ...
  return sendInteractiveMessage(m, sock, {
    title: `🔄 ${config.bot?.name || "SHIROWAHD"} AutoJPM`,
    body,
    footer: `${config.bot?.name || "SHIROWAHD"} AutoJPM`,
    buttons,
  });
}
...
async function handleSetDelay(m, sock, db, input) {
  ...
  if (!input) {
    ...
    return sendInteractiveMessage(m, sock, {
      title: `⏱️ ${config.bot?.name || "SHIROWAHD"} Delay`,
      body,
      footer: `${config.bot?.name || "SHIROWAHD"} JPM System`,
      buttons,
    });
  }
