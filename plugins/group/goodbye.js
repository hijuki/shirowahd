...
import { createGoodbyeCard } from "../../src/lib/hillz-welcome-card.js";
...
function resolvePlaceholders(
  ...
  return template
    ...
    .replace(/{bot}/gi, config.bot?.name || "SHIROWAHD")
...
async function sendGoodbyeMessage(sock, groupJid, participant, groupMeta) {
  try {
    ...
    if (goodbyeType === 2) {
      ...
      await sock.sendMessage(groupJid, {
        video: getAssetBuffer("hillz-mp4") || { url: "https://files.catbox.moe/k28dhp.mp4" },
        gifPlayback: true,
        caption: text,
        contextInfo: {
          mentionedJid: [realParticipant],
        ...
        viewOnceMessage: {
          message: {
            ...
            interactiveMessage: {
              ...
              footer: {
                text: config.bot?.name || "SHIROWAHD"
              },
              contextInfo: {
                mentionedJid: [realParticipant],
                isForwarded: true,
