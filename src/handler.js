...
async function getSmartTriggerThumb() {
  ...
  try {
    const url = config.assets["hillz2"];
    if (url) {
          ...
          try {
            await sock.sendPreview(
              m.chat,
              {
                ...
                jpegThumbnail: await sharp(fs.readFileSync(config.assets["hillz2"])).resize(300, 300).toBuffer(),
                previewType: 1,
              },
              { quoted: m }
            );
    ...
    for (let participant of participants) {
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: config.assets["hillz-promote"],
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: config.assets["hillz-demote"],
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
            ...
            viewOnceMessage: {
              message: {
                ...
                interactiveMessage: {
                  ...
                  footer: { text: config.bot?.name || "SHIROWAHD" },
