...
async function wallpaperScraper(query) {
  try {
    ...
    $('li[itemprop="associatedMedia"]').each((_, el) => {
      ...
      if (image && page) results.push({ title, resolution, image, page: "https://www.wallpaperflare.com" + page });
    });
    if (!results.length) return { success: false, message: "Tidak ditemukan wallpaper" };
