...
async function txt2img(prompt, style = "anime") {
  ...
  const { data: resultHtml } = await client.post("https://unrestrictedaiimagegenerator.com/", form.toString(), {
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
  });
  const $$ = cheerio.load(resultHtml);
  const imgSrc = $$(".generated-image img").attr("src") || $(".image-result img").attr("src") || $("img[src*='uploads']").attr("src");
  if (!imgSrc) throw new Error("Image not generated");
  return { status: true, url: imgSrc.startsWith("http") ? imgSrc : "https://unrestrictedaiimagegenerator.com" + imgSrc };
}
