...
const keys = [
  ["hillz-games", (buf) => { gameThumbBuffer = buf; }],
  ["hillz-rpg", (buf) => { rpgThumbBuffer = buf; }],
  ["hillz-winner", (buf) => { winnerThumbBuffer = buf; }],
];
...
function createFakeQuoted(botName = "HILLZ", verified = true) {
  return {
    key: {
      fromMe: false,
