          ...
          if (group.scheduleOpen === currentTime) {
            try {
              ...
              if (
                ...
                } catch (e) { console.error('[Scheduler] auto-open notify failed:', e.message); }
              } else {
                logger.error(
                  "GroupSchedule",
          ...
          if (group.scheduleClose === currentTime) {
            try {
              ...
              if (
                ...
                } catch (e) { console.error('[Scheduler] auto-close notify failed:', e.message); }
              } else {
                logger.error(
                  "GroupSchedule",
      ...
      for (const [groupId, data] of Object.entries(sewaGroups)) {
        ...
        if (remaining <= ONE_HOUR_MS && !data._warned1h) {
          try {
            ...
            warnedCount++;
            await new Promise((r) => setTimeout(r, 2000));
          } catch (e) { console.error('[Scheduler] 1h warn send failed:', e.message); }
        } else if (
          ...
          try {
            ...
            warnedCount++;
            await new Promise((r) => setTimeout(r, 2000));
          } catch (e) { console.error('[Scheduler] 3d warn send failed:', e.message); }
        }
      }
