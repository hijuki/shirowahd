...
footer: Powered by SHIROWAHD (opsional)
[/RICH:CODE]
...
[RICH:LIST]
title: Info Bot
rows: Nama | SHIROWAHD;; Versi | 2.4.5;; Developer | HILLZ
footer: © SHIROWAHD (opsional)
[/RICH:LIST]

5. STICKER (saat user minta sticker, atau untuk ekspresi emosi):
[RICH:STICKER]
url: https://iili.io/BPBdFuj.md.jpg
packname: SHIROWAHD (opsional)
  ...
  try {
    ...
    if (rich.type === "STICKER") {
      ...
      await sock.sendImageAsSticker(jid, stickerInput, quoted, {
        packname: packname || config.bot?.name || "SHIROWAHD",
    ...
    case "STICKER": {
      ...
      await sock.sendMessage(
        m.chat,
        {
          ...
          packname: config.bot?.name || "SHIROWAHD",
          author: "AutoAI",
        },
        { quoted: m },
      );
