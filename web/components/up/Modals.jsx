      ...
      <div className="absolute inset-0 z-10 overflow-y-auto modal-scroll-hide" onClick={close}>
        <div className="min-h-full flex p-4 py-6">
          <div
            ...
            className={`m-auto relative w-full max-w-[460px] transform-gpu will-change-transform ${closing ? 'modal-card-out' : 'modal-card-in'}`}
          >
            {/* Card */}
...
export function LinkButtons({ ownerWhatsapp, channels = [], claimGroups = [], popupButtons = [], settings = {} }) {
  ...
  if (settings.showClaimBtn !== false) claimGroups.slice(0, 2).forEach(g => items.push({ i: 'fa-solid fa-comments', t: g.name || 'Grup Claim', s: 'Klaim file di sini', href: g.link, c: '#38bdf8' }))
...
export function IntroModal({ onDone, settings }) {
  ...
  return (
    <ModalShell onClose={onDone}>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-[22px]">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="confetti-particle" style={{
            '--x': `${8 + Math.random() * 84}%`,
            '--delay': `${0.2 + i * 0.15}s`,
            '--color': ['#22d3ee', '#3b82f6', '#67e8f9', '#38bdf8', '#22d3ee'][i % 5],
      ...
      {/* ── CTA utama dulu (sesuai alur: upload → kode → claim), link resmi jadi sekunder ── */}
      <div className={`transform-gpu transition-[transform,opacity] duration-[500ms] ${phase >= 4 ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'}`} style={{ transitionTimingFunction: 'var(--ease-out)' }}>
        ...
        {(settings?.ownerWhatsapp || settings?.channels?.length || settings?.claimGroups?.length || settings?.popupButtons?.length) ? (
          <>
            <p className="text-[8px] font-extrabold tracking-[.18em] uppercase text-[#7e90ad]/60 mt-5 mb-2 flex items-center gap-2">
              <span className="flex-1 h-px bg-white/[.06]" />Link Resmi<span className="flex-1 h-px bg-white/[.06]" />
            </p>
        ...
        <p className="text-center text-[#7e90ad]/50 text-[8px] mt-3 tracking-wide">© {new Date().getFullYear()} SWHDHLZ · BY HILLZ</p>
      </div>
    </ModalShell>
  )
}
    ...
    <ModalShell onClose={onClose}>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[{ i: 'fa-bolt', t: 'Cepat', c: '#22d3ee' }, { i: 'fa-lock', t: 'Aman', c: '#38bdf8' }, { i: 'fa-wand-magic-sparkles', t: 'Mudah', c: '#3b82f6' }].map(f => (
